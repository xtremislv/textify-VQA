...
def prepare_dataset(df, download_folder):

    os.makedirs(download_folder, exist_ok=True)
    image_links = df["image_link"]

    df["file_name"] = df["image_link"].apply(lambda x: Path(x).name)
    df = df[["file_name", "entity_name", "entity_value"]]

    df["json"] = df.to_json(orient="records", lines=True).splitlines()

    with open(os.path.join(download_folder, "metadata.jsonl"), "w") as f:
        f.writelines(df["json"])
